<template>
  <entity-page narrow
               caption="Authorization Role Assignments"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const loadAssignments = async (action: string, id: string) => {
      principals.value = await authorizationService.getRolePrincipals(
        id,
        action === 'view',
      )
    }
      ...
      quasar.notify({
        ...
        actions: [
          {label: 'Cancel', color: 'white'},
          {
            ...
            handler: async () => {
              // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
              await authorizationService.unassignPrincipal(entityPage.id.value, principal)
