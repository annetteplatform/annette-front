...
export default defineComponent({
  ...
  setup(props) {
    ...
    const loadAssignedOrgRoles = async () => {
      ...
      const roles: OrgRole[] = await store.dispatch('orgRole/loadEntitiesIfNotExist', entity.value.orgRoles)
    ...
    const assignOrgRole = async (newOrgRoleId: string) => {
      ...
      await await store.dispatch('orgItem/assignOrgRole', payload)
      orgRoleId.value = ''
    }
    ...
    const unassignOrgRole = async (orgRoleId: string) => {
      ...
      await store.dispatch('orgItem/unassignOrgRole', payload)
    }
