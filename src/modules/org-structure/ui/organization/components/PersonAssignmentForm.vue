...
import {
  ...
  OrgPosition,
  UnassignPersonPayloadDto, useOrgItemStore
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const loadAssignedPersons = async () => {
      ...
      const persons: Person[] = await personStore.loadEntitiesIfNotExist(entity.value.persons)
    ...
    const changePositionLimit = async (limit: string) => {
      ...
      await orgItemStore.changePositionLimit(payload)
    ...
    const assignPerson = async (newPersonId: string) => {
      ...
      await orgItemStore.assignPerson(payload)
      personId.value = ''
    }
    ...
    const unassignPerson = async (removedPersonId: string) => {
      ...
      await orgItemStore.unassignPerson(payload)
    }
