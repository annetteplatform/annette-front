<template>
  <entity-page narrow
               :caption="$t('annette.serviceCatalog.category.title')"
    ...
    <template v-slot:toolbar>
      <default-entity-page-toolbar :action="action" :id="id"
                                   ...
                                   :back-label="$t('annette.serviceCatalog.category.titlePl')"
                                   back-route-name="serviceCatalog.categories"/>
    </template>
    ...
    <template v-slot:save-toolbar>
      <div class="row">
        <q-btn v-if="action == 'edit'" outline dense
               ...
               :to="{ name: 'serviceCatalog.category', params: { action: 'view', id } }"/>
        <q-btn dense
               color="primary"
    ...
    <template v-slot:default>
      <div class="row q-pb-md">
        <q-input class="col-md-4 col-sm-12 col-xs-12 " stack-label dense
                 ...
                 ref="idRef"
                 :label="$t('annette.serviceCatalog.category.field.id')"/>
      </div>
      <div class="row q-pb-md">
        <q-input class="col-md-12 col-sm-12 col-xs-12 " stack-label dense
                 ...
                 ref="nameRef"
                 :label="$t('annette.serviceCatalog.category.field.name')"/>
      </div>
