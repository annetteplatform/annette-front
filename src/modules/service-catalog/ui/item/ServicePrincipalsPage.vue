  ...
  <entity-page narrow
    ...
    <template v-slot:toolbar>
      <q-btn class="q-mr-md" outline color="primary"
             label="Service Items"
             :to="{name: 'serviceCatalog.serviceItems'}"/>
      ...
      <q-btn v-if="action === 'edit'"
             class="q-mr-md" outline color="primary"
             label="Service item"
             :to="{ name: 'serviceCatalog.serviceItem', params: { action: 'edit', id } }"/>
      <q-btn v-if="action === 'view'"
             class="q-mr-md" outline color="primary"
             label="Service item"
...
import {
  ...
  ServiceItem,
  useServiceItemStore
} from 'src/modules/service-catalog';


export default defineComponent({
  name: 'ServicePrincipalsPage',
  ...
  setup(props) {
    ...
    const loadAssignments = async (action: string, id: string) => {
      principals.value = await serviceCatalogService.getServicePrincipals(id)
    }
    ...
    const addPrincipal = async () => {
      ...
      await serviceCatalogService.assignServicePrincipal(entityPage.id.value, principal)
      ...
      quasar.notify({
        ...
        actions: [
          {label: 'Cancel', color: 'white'},
          {
            ...
            handler: async () => {
              // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
              await serviceCatalogService.unassignServicePrincipal(entityPage.id.value, principal)
