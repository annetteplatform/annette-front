    ...
    <template v-slot:row="props">
      ...
      <q-td auto-width>
        <q-btn flat round color="green" size="sm" icon="far fa-eye"
               :to="{ name: 'principalGroup.group', params: { action: 'view', id: props.row.id } }"/>
        <q-btn flat round color="blue" size="sm" icon="far fa-edit"
               :to="{ name: 'principalGroup.group', params: { action: 'edit', id: props.row.id } }"/>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const entityList = useEntityList<PrincipalGroup, PrincipalGroupFilter>(
      NAMESPACE,
      props.instanceKey,
      'Please confirm delete principal group.'
    )
