<template>
  <entity-page narrow
               :caption="$t('annette.person.category.title')"
    ...
    <template v-slot:toolbar>
      <default-entity-page-toolbar :action="action" :id="id"
                                   ...
                                   :back-label="$t('annette.person.category.titlePl')"
                                   back-route-name="person.categories"
                                   @save="save"/>
    </template>
    ...
    <template v-slot:save-toolbar>
      <div class="row">
        <q-btn v-if="action == 'edit'" outline dense
               ...
               :to="{ name: 'person.category', params: { action: 'view', id } }"/>
        <q-btn dense
               color="primary"
    ...
    <template v-slot:default>
      <div class="row q-pb-md">
        <q-input class="col-md-4 col-sm-12 col-xs-12 " stack-label dense
                 ...
                 ref="idRef"
                 :label="$t('annette.person.category.field.id')"/>
      </div>
      <div class="row q-pb-md">
        <q-input class="col-md-12 col-sm-12 col-xs-12 " stack-label dense
                 ...
                 ref="nameRef"
                 :label="$t('annette.person.category.field.name')"/>
      </div>
