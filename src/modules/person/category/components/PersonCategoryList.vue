...
export default defineComponent({
  ...
  setup(props) {
    ...
    const entityList = useEntityList<Category, CategoryFilter>('personCategory', props.instanceKey)

    const deleteEntity = (id: string) => {
      quasar.notify({
        type: 'negative',
        message: 'Please confirm delete category.',
        actions: [
          {label: 'Cancel', color: 'white'},
          {
            ...
            color: 'white',
            handler: () => {
              void store.dispatch('personCategory/deleteEntity', id)
            }
          }
        ]
      })
