    ...
    <template v-slot:row="props">
      ...
      <q-td auto-width>
        <q-btn flat round color="green" size="sm" icon="far fa-eye"
               :to="{ name: 'application.translation', params: { action: 'view', id: props.row.id } }"/>
        <q-btn flat round color="blue" size="sm" icon="far fa-edit"
               :to="{ name: 'application.translation', params: { action: 'edit', id: props.row.id } }"/>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const entityList = useEntityList<Translation, TranslationFilter>(
      NAMESPACE,
      props.instanceKey,
      'Please confirm delete translation.'
    )
